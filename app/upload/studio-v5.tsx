...
import { ArrowLeft, Camera, Check, Clock3, Flashlight, Image as ImageIcon, Music2, RotateCcw, Settings2, Sparkles, SwitchCamera, Upload, Video, X, Zap } from "lucide-react";
...
export default function StudioV5() {
  ...
  function dims() {
    ...
    const base = 720; return r.w >= r.h ? [base, Math.round(base * r.h / r.w)] : [Math.round(base * r.w / r.h), base];
  }
  ...
  function takePhoto() {
    ...
    ctx.save(); if (facing === "user") { ctx.translate(w, 0); ctx.scale(-1, 1); } ctx.drawImage(v, (w - dw) / 2, (h - dh) / 2, dw, dh); ctx.restore();
    setResult(c.toDataURL("image/jpeg", .92)); setResultType("image"); streamRef.current?.getTracks().forEach(t => t.stop());
  }
  ...
  function startVideo() {
    ...
    const draw = () => {
      ctx.fillStyle = "#000"; ctx.fillRect(0, 0, w, h); ctx.filter = liveFilter;
      const scale = Math.min(w / (v.videoWidth || w), h / (v.videoHeight || h)); const dw = (v.videoWidth || w) * scale, dh = (v.videoHeight || h) * scale;
      ctx.save(); if (facing === "user") { ctx.translate(w, 0); ctx.scale(-1, 1); } ctx.drawImage(v, (w - dw) / 2, (h - dh) / 2, dw, dh); ctx.restore();
      if (recording) requestAnimationFrame(draw);
    };
    draw();
    const out = canvas.captureStream(30); streamRef.current?.getAudioTracks().forEach(t => out.addTrack(t));
    const rec = new MediaRecorder(out, { mimeType: type }); chunksRef.current = [];
    rec.ondataavailable = e => e.data.size && chunksRef.current.push(e.data);
    rec.onstop = () => { const blob = new Blob(chunksRef.current, { type }); setResult(URL.createObjectURL(blob)); setResultType("video"); setRecording(false); streamRef.current?.getTracks().forEach(t => t.stop()); out.getTracks().forEach(t => t.stop()); };
    recorderRef.current = rec; rec.start(250); setRecording(true); setSeconds(0);
    const started = Date.now(); const tick = () => { if (recorderRef.current === rec && rec.state === "recording") { setSeconds(Math.floor((Date.now() - started) / 1000)); requestAnimationFrame(tick); } }; tick();
  }

  function shutter() {
    if (recording) { recorderRef.current?.stop(); recorderRef.current = null; return; }
    if (mode === "photo") return takePhoto();
    if (countdown > 0) { setCountLeft(countdown); let n = countdown; const id = setInterval(() => { n--; setCountLeft(n); if (n <= 0) { clearInterval(id); startVideo(); } }, 1000); return; }
    startVideo();
  }
  ...
  async function publish() {
    ...
    try {
      const res = await fetch(result); const blob = await res.blob(); const ext = resultType === "image" ? "jpg" : (blob.type.includes("mp4") ? "mp4" : "webm");
