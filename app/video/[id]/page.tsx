...
import { ArrowLeft, Heart, MessageCircle, Share2, Bookmark, Send, UserRound, ChevronUp, ChevronDown } from "lucide-react";
import { supabase } from "../../../lib/supabase";
import "./detail.css";

type Video={id:string;url:string;title:string|null;music:string|null;cover_url:string|null;like_count:number;comment_count:number;user_id:string;created_at:string;profile:any};
...
const videoFields="id,url,title,music,cover_url,like_count,comment_count,user_id,created_at";
...
export default function VideoPage(){
 ...
 async function submit(e:FormEvent){e.preventDefault();const value=text.trim();if(!value||!video||!supabase)return;if(!userId)return needLogin();setSending(true);const {data,error}=await supabase.from("comments").insert({video_id:video.id,user_id:userId,content:value}).select("id,content,created_at,user_id").single();if(!error&&data){setComments(c=>[{...data,username:"我",avatar_url:null},...c]);setVideos(vs=>vs.map(v=>v.id===video.id?{...v,comment_count:v.comment_count+1}:v));setText("");await supabase.from("video_events").insert({user_id:userId,video_id:video.id,event_type:"comment"})}else setToast("评论失败，请稍后重试");setSending(false)}
 async function share(){if(!video)return;const url=`${window.location.origin}/video/${video.id}`;if(navigator.share)await navigator.share({title:video.title||"星流视频",url}).catch(()=>{});else{await navigator.clipboard?.writeText(url);setToast("链接已复制")}}
 async function save(){if(!video||!supabase)return;if(!userId)return needLogin();const next=!saved;setSaved(next);const q=next?supabase.from("saved_videos").insert({user_id:userId,video_id:video.id}):supabase.from("saved_videos").delete().eq("user_id",userId).eq("video_id",video.id);const{error}=await q;if(error){setSaved(!next);setToast("收藏失败："+error.message);return}await supabase.from("video_events").insert({user_id:userId,video_id:video.id,event_type:next?"save":"unsave"});setToast(next?"已收藏":"已取消收藏")}
