  ...
  return <main className="page">
    <style jsx global>{`
      *{box-sizing:border-box}
      ...
      .hero{height:423px;position:relative;overflow:hidden;color:#fff;background:linear-gradient(180deg,#8bb7d0 0%,#527c95 38%,#34474d 73%,#252a2d 100%)}
      ...
      .top{position:absolute;z-index:4;top:10px;left:0;right:0;padding:0 18px;display:flex;align-items:center;justify-content:space-between}
      .topRight{display:flex;gap:8px}
      .glass{height:43px;border:0;border-radius:23px;background:#17232c66;color:#fff;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(8px);box-shadow:inset 0 0 0 1px #ffffff18}
      .pill{padding:0 15px;gap:6px;font-weight:650}.icon{width:43px}
      .heroInfo{position:absolute;z-index:3;left:29px;right:24px;bottom:28px;display:flex;align-items:flex-end;gap:28px}
      .avatarWrap{position:relative;flex:none}
      .avatar{width:187px;height:187px;border-radius:50%;object-fit:cover;background:#05050a;border:3px solid #fff;box-shadow:0 3px 18px #0008}
      .add{position:absolute;right:2px;bottom:5px;width:48px;height:48px;border-radius:50%;border:3px solid #fff;background:#35cf55;color:#fff;display:grid;place-items:center}
      .identity{padding-bottom:13px;text-shadow:0 2px 7px #0009}.nameRow{display:flex;align-items:center;gap:8px}.name{font-size:30px;font-weight:850}.badge{background:#f32e58;border-radius:12px;padding:2px 7px;font-size:13px;font-weight:800}.id{font-size:14px;color:#ddd;margin-top:9px}.ai{margin-top:12px;width:max-content;padding:8px 14px;border:0;border-radius:20px;background:#ffffff22;color:#fff;font-weight:750;backdrop-filter:blur(8px)}
      .sheet{position:relative;z-index:5;margin-top:-1px;background:#fff;border-radius:17px 17px 0 0;min-height:650px;padding-top:8px}
      .stats{display:grid;grid-template-columns:repeat(4,1fr);align-items:center;height:84px}.stat{text-align:center}.stat b{display:block;font-size:25px;line-height:1;font-weight:850}.stat span{display:block;margin-top:7px;color:#696b72;font-size:14px}
      .edit{height:48px;margin:0 29px 8px;border:0;border-radius:8px;background:#f4f4f6;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:16px}
      .bioRow{padding:7px 29px 2px;color:#686a70;font-size:16px;display:flex;align-items:center;gap:5px}.bioRow a{color:#666}
      .tags{padding:8px 29px 14px;display:flex;gap:7px;flex-wrap:wrap}.tag{border:0;background:#f5f5f6;border-radius:5px;color:#777;padding:5px 9px;font-size:13px}.tag.addTag{border:1px solid #eee;background:#fafafa}
      .quick{display:grid;grid-template-columns:repeat(5,1fr);padding:12px 10px 16px}.q{border:0;background:none;display:flex;flex-direction:column;align-items:center;gap:9px;color:#25262b;font-size:13px}.q svg{width:30px;height:30px;stroke-width:1.7}
      .promo{margin:0 29px 14px;height:74px;border:1px solid #e8e8ea;border-radius:9px;display:flex;align-items:center;padding:9px 10px;gap:11px;position:relative;overflow:hidden}.promoImg{width:62px;height:54px;border-radius:6px;background:linear-gradient(145deg,#111,#444);display:grid;place-items:center;color:#fff}.promoText{flex:1}.promoText b{display:block;font-size:15px}.promoText span{display:block;color:#888;margin-top:5px;font-size:13px}.promoBtn{border:0;border-radius:7px;background:#fff0f3;color:#ef3b65;font-weight:800;padding:10px 13px}.promoX{position:absolute;right:4px;top:4px;border:0;background:none;color:#aaa}
      .tabs{height:62px;display:grid;grid-template-columns:repeat(5,1fr);border-bottom:1px solid #eee;position:sticky;top:0;background:#fffffff8;z-index:10}.tab{border:0;background:none;color:#777;font-size:16px;position:relative}.tab.on{color:#17181d;font-weight:850}.tab.on:after{content:"";position:absolute;left:50%;bottom:0;transform:translateX(-50%);height:3px;width:46px;background:#17181d;border-radius:4px}
      .private{margin:14px 29px 21px;height:65px;border:1px solid #e6e7e9;border-radius:8px;display:flex;align-items:center;padding:0 14px;gap:10px}.privateThumb{width:53px;height:45px;border-radius:4px;background:linear-gradient(135deg,#ddd,#777);display:grid;place-items:center;color:#fff}.privateText{font-weight:800}.privateText small{display:block;color:#999;font-weight:400;margin-top:4px}.privateCount{margin-left:auto;color:#777;display:flex;align-items:center}
      .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:2px;background:#f3f3f3}.tile{position:relative;aspect-ratio:3/4;background:#ddd;overflow:hidden}.tile img,.tile video{width:100%;height:100%;object-fit:cover}.tileShade{position:absolute;inset:55% 0 0;background:linear-gradient(transparent,#0008)}.draft{position:absolute;left:8px;top:8px;background:#fff;color:#333;padding:5px 8px;border-radius:5px;font-size:12px;font-weight:750}.tileMeta{position:absolute;bottom:7px;left:8px;color:#fff;font-size:12px;text-shadow:0 1px 5px #000}.empty{text-align:center;padding:75px 20px;color:#999}.empty a{display:inline-block;margin-top:10px;color:#f13b62;font-weight:750}
      .bottom{position:fixed;z-index:30;left:0;right:0;bottom:0;height:74px;background:#fff;border-top:1px solid #eee;display:grid;grid-template-columns:repeat(5,1fr);padding-bottom:env(safe-area-inset-bottom)}.nav{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;font-weight:700;font-size:12px;color:#676870}.nav.active{color:#17181d}.publish{width:48px;height:42px;border:2px solid #17181d;border-radius:11px;display:grid;place-items:center;background:#fff}
      ...
      @media(max-width:520px){.heroInfo{left:28px;gap:20px}.avatar{width:152px;height:152px}.name{font-size:28px}.identity{padding-bottom:7px}.sheet{border-radius:16px 16px 0 0}.stats{height:82px}.edit{margin-left:29px;margin-right:29px}.bioRow,.tags{padding-left:29px;padding-right:29px}}
    `}</style>
